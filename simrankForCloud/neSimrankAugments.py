...
dataWithAugmentsPredicate = pd.read_csv("augments_freq_5.csv",
                                 delimiter=';',error_bad_lines=False, encoding='unicode_escape')

print(f'Dataset Threats Shape: {dataWithAugmentsPredicate.shape}')

print(dataWithAugmentsPredicate.head())
data = dataWithAugmentsPredicate.head(27335)
print(data.subject_cui.unique().size)
...
s_airports.to_parquet('simrankOfGraphOfAugments.parquet', index=True)
    ...
    for index2 in range(index + 1, len(drugsWithCUI)):
        if not pd.isnull(row["cui"]) and not pd.isnull(drugsWithCUI.iloc[index2]['cui']):
            try :
                ...
                with open('simrankScoresAugments.csv', 'a', newline='') as fd:
