...
class SimRank(object):
    ...
    def _create_graph(self, data, weighted, from_node_column, to_node_column, weight_column):
        ...
        self.Graph = pd.DataFrame(np.zeros((len(self.Nodes), len(self.Nodes))), list(self.Nodes), list(self.Nodes))
        if weighted:
            inNeighbors = data.groupby(to_node_column)[weight_column].sum().to_frame().rename(
                columns={weight_column: 'inNeighbors'})
        else:
            inNeighbors = data.groupby(to_node_column)[from_node_column].count().to_frame().rename(
                columns={from_node_column: 'inNeighbors'})
...
class BipartiteSimRank(object):
    ...
    def _create_graph(self, data, weighted, node_group1_column, node_group2_column, weight_column):
        ...
        self.Graph_N1_N2 = pd.DataFrame(np.zeros((len(self.NodesGroup1), len(self.NodesGroup2))),
                                        index=self.NodesGroup1, columns=self.NodesGroup2)
        self.Graph_N2_N1 = pd.DataFrame(np.zeros((len(self.NodesGroup2), len(self.NodesGroup1))),
                                        index=self.NodesGroup2, columns=self.NodesGroup1)
        if weighted:
            NodeGroup1Neighbors = data.groupby(node_group1_column)[weight_column].sum().to_frame().rename(
                columns={weight_column: 'node_group1_neighbors'})
            NodeGroup2Neighbors = data.groupby(node_group2_column)[weight_column].sum().to_frame().rename(
                columns={weight_column: 'node_group2_neighbors'})
        else:
            NodeGroup1Neighbors = data.groupby(node_group1_column)[node_group2_column].count().to_frame().rename(
                columns={node_group2_column: 'node_group1_neighbors'})
            NodeGroup2Neighbors = data.groupby(node_group2_column)[node_group1_column].count().to_frame().rename(
                columns={node_group1_column: 'node_group2_neighbors'})
        data = data.join(NodeGroup1Neighbors, on=node_group1_column).join(NodeGroup2Neighbors, on=node_group2_column)
        data['n1_n2_normailized_weight'] = (1.0 / data['node_group1_neighbors']).replace([np.inf, -np.inf],
                                                                                         np.nan).fillna(0)
        data['n2_n1_normailized_weight'] = (1.0 / data['node_group2_neighbors']).replace([np.inf, -np.inf],
                                                                                         np.nan).fillna(0)
        self.Graph_N1_N2 = data.pivot(index=node_group1_column, columns=node_group2_column,
                                      values='n1_n2_normailized_weight').fillna(0)
        self.Graph_N2_N1 = data.pivot(index=node_group2_column, columns=node_group1_column,
                                      values='n2_n1_normailized_weight').fillna(0)
    ...
    def fit(self, data, C1=0.8, C2=0.8, weighted=False, node_group1_column='user', node_group2_column='item',
        ...
        for _iter in range(iterations):
            ...
            np.fill_diagonal(new_S_N2, 1)
        return pd.DataFrame(new_S_N1, index=self.NodesGroup1, columns=self.NodesGroup1), pd.DataFrame(new_S_N2,
                                                                                                      index=self.NodesGroup2,
                                                                                                      columns=self.NodesGroup2)
...
class SimRankPP(SimRank):
    ...
    def fit(self, data, C=0.8, weighted=False, from_node_column='from', to_node_column='to', weight_column='weight',
            iterations=100, eps=1e-10, verbose=True):
        self._create_graph(data, weighted, from_node_column, to_node_column, weight_column)
        ...
        for _iter in range(iterations):
            ...
            np.fill_diagonal(new_S, 1)
        return pd.DataFrame(new_S, list(self.Nodes), list(self.Nodes))
...
class AprioriSimRank(SimRankPP):
    ...
    def fit(self, data, AprioriSim, C=0.8, lbd=0.5, weighted=False, from_node_column='from', to_node_column='to',
            weight_column='weight', iterations=100, eps=1e-4, verbose=True):
        self._create_graph(data, weighted, from_node_column, to_node_column, weight_column)
        # Cal W for user and item
        self.Weight = self._cal_Weight(self.Graph, verbose)
...
class BipartitleAprioriSimRank(BipartiteSimRankPP):
    ...
    def fit(self, data, AprioriSim1, AprioriSim2, C1=0.8, C2=0.8, lbd1=0.5, lbd2=0.5, weighted=False,
            node_group1_column='user', node_group2_column='item', weight_column='weight', iterations=100, eps=1e-4,
            verbose=True):
        self._create_graph(data, weighted, node_group1_column, node_group2_column, weight_column)
        # Cal W for user and item
        self.Weight_N1 = self._cal_Weight(self.Graph_N1_N2, verbose)
        ...
        for _iter in range(iterations):
            ...
            if verbose:
                update_progress(_iter / iterations)
            old_S_N1 = copy.deepcopy(new_S_N1)
            new_S_N1 = (1 - lbd1) * self.Evidence_N1 * C1 * self.Weight_N1.dot(new_S_N2).dot(
                self.Weight_N1.T) + lbd1 * AprioriSim1
            np.fill_diagonal(new_S_N1, 1)
            old_S_N2 = copy.deepcopy(new_S_N2)
            new_S_N2 = (1 - lbd2) * self.Evidence_N1 * C2 * self.Weight_N2.dot(new_S_N1).dot(
                self.Weight_N2.T) + lbd2 * AprioriSim2
            np.fill_diagonal(new_S_N2, 1)
        return pd.DataFrame(new_S_N1, index=self.NodesGroup1, columns=self.NodesGroup1), pd.DataFrame(new_S_N2,
                                                                                                      index=self.NodesGroup2,
                                                                                                      columns=self.NodesGroup2)